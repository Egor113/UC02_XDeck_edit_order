	...
	public void addTask() throws Throwable{
		try (Statement statement = connection.createStatement()) {
            ...
            try {
            	...
            	String insert_query = "INSERT INTO task (id, change_id, ticket_id, guid, header, text, priority_id, state_id, client_id, create_date, external_system)values ('" + lr.eval_string("{ID}").trim() + "','IDC2D620524153zdzPWAoX9OFgW4UB', '"+ lr.eval_string("{ID}").trim() +
